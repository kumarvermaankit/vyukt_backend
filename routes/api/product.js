...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const designUrl = req.body.imageUrl;
      const color = req.body.color;
      const user_consent = req.body.consent;
     
      console.log(sku)
      if (!sku) {
        console.log('You must enter sku.')
        return res.status(400).json({ error: 'You must enter sku.' });
      }

      if (!description || !name) {
        console.log('You must enter description & name.')
        return res
          .status(400)
      ...
      if (foundProduct) {
        console.log('This sku is already in use.')
